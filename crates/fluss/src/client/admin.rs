...
mod tests {
    ...
    use crate::metadata::{
        DataField, DataTypes, DatabaseDescriptor, JsonSerde, Schema, TableDescriptor, TableInfo,
        TablePath,
    };
    use crate::proto::{
    ...
    fn build_table_info(table_path: TablePath, table_id: i64) -> TableInfo {
        ...
        TableInfo::of(table_path, table_id, 1, table_descriptor, 10, 20)
    }
    ...
    async fn admin_requests_round_trip() -> Result<()> {
        ...
        let cluster = build_cluster(&table_path, table_id, coordinator.clone(), tablet.clone());
    ...
    async fn list_offsets_empty_buckets_error() -> Result<()> {
        ...
        let cluster = build_cluster(&table_path, 1, coordinator, tablet);
        let metadata = Arc::new(Metadata::new_for_test(cluster));
        let rpc_client = Arc::new(RpcClient::new());
